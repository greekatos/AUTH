...
document.addEventListener("DOMContentLoaded", () => {
  ...
  for (let i = 0; i < sort_data.length; i++) {
    ...
    const eventImage = images[i];
    const eventLink = links[i];
    eventDate.innerHTML = sort_data[i].calendar;
    eventTitle.innerHTML = sort_data[i].eventName;
    eventImage.src = sort_data[i].image;
    eventLink.href = ` ${sort_data[i].link}?uuid=${sort_data[i].id}`;
  }
});
